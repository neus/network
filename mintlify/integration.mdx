...
Recommended end-to-end path for production: check gate eligibility, run verification, poll status, unlock access. Use hosted verify for interactive flows.
...
For the simplest flow: use hosted verify + auth code exchange.

1. Redirect users to `https://neus.network/verify?intent=login&returnUrl=...`
2. After sign-in, the page returns an auth `code`
3. When partner code exchange is enabled for your integration, your server calls `POST https://api.neus.network/api/v1/auth/code/exchange` with the code and the server credential issued during onboarding
...
- **Sponsored flows:** include `X-Neus-App` (public appId) and `X-Sponsor-Grant` when your server is covering verification costs.

## Policy-Based Gate Checks

For server-side policy enforcement, use `gateCheck` with explicit recency:
...
<Checklist>
  <Item>Use `verifiers=` or `preset=` in hosted URLs; avoid hardcoding verifier metadata.</Item>
  <Item>For popup flows, pass `origin=` and listen for `neus_checkout_done`.</Item>
  <Item>Choose billing model: self-pay, sponsor-grant, or x402 pay-per-use.</Item>
  <Item>Run a golden demo: create proof → obtain `qHash` → open `/proof/[qHash]`.</Item>
</Checklist>
...
<CardGroup cols={3}>
  ...
  <Card title="Presets" icon="layer-group" href="./platform/presets">
    Preset catalog for hosted verify URLs.
  </Card>

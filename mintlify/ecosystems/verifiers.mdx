...
|--------|-----------|----------|
| **AI trust receipt** | `ownership-basic` + `ai-content-moderation` + `proof-of-human` | AI-generated content with human attestation |
...
| **Org access** | `ownership-org-oauth` + `ownership-dns-txt` + `contract-ownership` | Organization verification and domain control |
...
Verify social account ownership via OAuth. Requires hosted checkout. For programmatic flows, use the session token returned from the hosted flow at `/verify`—not a raw provider token.

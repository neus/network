...
export function createNeusMockCore(options = {}) {
  ...
  function handle(method, pathname, _search, bodyText) {
    ...
    if (proofGet) {
      ...
      // Local-only: hosted mock checkout invents a fresh qHash without a prior POST /verification.
      // Return a minimal verified payload so ProofBadge can render during UI iteration.
      return {
        status: 200,
        json: {
          success: true,
          data: {
            ...
            qHash: id,
            walletAddress: '',
            verifiedVerifiers: [{ verifierId: 'mock-handoff', verified: true }]
          }
        }
      };
    }

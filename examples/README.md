# Examples

| Example | Use it for |
| ------- | ---------- |
| [trust-receipts-showcase](./trust-receipts-showcase/README.md) | End-to-end **VerifyGate** + gate check demo |
| [nodejs-basic](./nodejs-basic/README.md) | Server-side receipt creation and checks |

Run the showcase locally, then follow the [Quickstart](https://docs.neus.network/quickstart) to add the same trust loop to your app.

Docs: [Get started](https://docs.neus.network/get-started) · [Quickstart](https://docs.neus.network/quickstart) · [Integration](https://docs.neus.network/integration) · [MCP](https://docs.neus.network/mcp/setup)

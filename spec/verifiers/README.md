...
**Request schemas** (`spec/verifiers/schemas/`) show the fields your app supplies in API calls. Hosted-only verifiers like `ownership-social` and `ownership-org-oauth` require only `provider` (and optional `expectedOrgDomain` for org). The `internalSocialToken` field is populated by the hosted OAuth flow and should not be supplied in your requests.

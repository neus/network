...
`VerifyGate` checks eligibility, sends users through hosted verification when needed, then renders children. New proofs default to **private**, matching `client.verify()`. See [Security and trust](../platform/security-and-trust) for visibility; use [Get started](../get-started) for production `appId` and linking.

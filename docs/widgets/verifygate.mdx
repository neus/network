...
`VerifyGate`: check to hosted verify when needed to unlock. Create defaults **unlisted public** so gates work without extra `proofOptions`. Raw `client.verify()` stays **private**. [Security and trust](../platform/security-and-trust) | [Get started](../get-started)
...
| `sponsorGrant` | Bill your org for customer traffic ([Billing](../platform/billing)) |

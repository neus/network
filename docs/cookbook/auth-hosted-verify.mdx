...
<Note>
**Default path:** users verify on NEUS → your app saves the **proof receipt ID** → your backend reuses it with checks ([Integration](../integration#loop)). **Most integrations:** your app pays via sponsor grants—users are not asked to manage billing first ([Get started](../get-started#choose-who-pays)).
</Note>
...
**MCP:** run **`neus_context`** first; tools return **`neus.network/verify`** links without requiring a key. Add **Bearer** with your **personal access key** from **[Access keys](https://neus.network/profile?tab=account)** when you need a signed-in MCP session. [MCP Auth](../mcp/auth)

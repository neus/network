---
title: Hosted Verify
description: `neus.network/verify` — login, checks, gates; ceremony on NEUS.
icon: key
---

**Hosted Verify** is `https://neus.network/verify`. Use it when a step needs a **browser ceremony** (passkey, wallet, OAuth, human check, or gate checkout). For the default path, your app does **not** embed wallet or passkey UI.
...
| `returnUrl` | Absolute `https://…` back to **your** app after completion (integrators). Validated before redirect. |
...
| `connect=1` | With `intent=login`, auto-start wallet connect on load |
...
Server-only receipts: [Quickstart](../quickstart) (`verifyFromApp`).

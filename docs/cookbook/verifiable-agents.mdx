...
description: Register an agent, grant its authority, and connect both proofs to its runtime.
icon: robot
---

Two results: **`agent-identity`** (agent signs) and **`agent-delegation`** (approving account signs). Same person or two accounts.

To bill hosted checks through your product, complete [Sell access](../quickstart) (create a **listing** under profile → **Listings**). For backend proof creation after user approval, see [Integrations](./integrations). Agent concepts: [Agent concepts](../agents/concepts)

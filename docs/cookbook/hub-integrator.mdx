...
Use this guide when you are connecting a website, community, app, team, or partner flow to NEUS. The public integration story is simple:

1. Use hosted verify as the standard guided flow.
2. Save the proof receipt ID.
3. Reuse that proof receipt for profile trust, onboarding, access checks, proof pages, and app attribution.
...
| Linked apps | Apps and platforms can attribute usage and connect a real integration surface to the same proof layer |
...
Link your domain to your wallet by setting a DNS TXT record. This is the foundation for site trust, partner onboarding, app linking, and namespaced identity surfaces.
...
<Note>
Most integrators do not need to create their own Google or Microsoft OAuth app just to use the hosted NEUS flow.
</Note>

## 3. Link the App or Site in Hub

Link your integration domain in the Hub to receive attribution, usage tracking, and credit controls:
...
## 5. Build the Portable Trust Surface

These live capabilities can already be combined into a portable identity and trust surface:
...
- proof pages and profile surfaces

In Hub, the most common flows are:
...
| Username | Profile / Claim flows | Use portable handle and namespace claim surfaces |
| Linked wallets | Profile identity flows | Treat multiple wallets as one reusable trust surface |
...
NEUS is most useful when you keep one clean mental model:

- a person has a reusable trust surface
- that surface can collect wallets, linked accounts, domains, team association, and proof-backed badges over time

...
const proof = await client.verify({
  ...
  data: {
    ...
    namespace: 'catalyst',         // isolate from default neus namespace
    displayName: 'Alice',
    metadata: { platform: 'catalyst' }
  },
...
const url = getHostedCheckoutUrl({
  gateId: 'gate_catalyst-handles',
  returnUrl: 'https://catalyst.network/handle/callback',
});
...
```bash
curl "https://api.neus.network/api/v1/proofs/check?gateId=gate_catalyst-handles&address=0x...&namespace=catalyst"
```

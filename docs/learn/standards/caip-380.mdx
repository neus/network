...
description: Move a verified result between apps without repeating the check.
icon: file-signature
---

CAIP-380 is an open [ChainAgnostic](https://standards.chainagnostic.org/CAIPs/caip-380) standard at **Draft** status. NEUS is the driving reference implementation. This page describes the portable envelope emitted for wallet-signed NEUS verification requests. See the [standards chronology](/whitepaper#standards-and-interoperability) for lifecycle status and advancement path.
...
The helper recomputes `qHash`, checks the DID and chain binding, reconstructs the six-line message, and verifies the signature locally. EVM uses EIP-191, EIP-1271, and EIP-6492. Non-EVM chains use their native signing scheme (Ed25519 for Solana, NEAR's `near_sign` for NEAR, and so on). EIP-1271 smart accounts require chain state; pass an ethers provider as `options.provider`.
...
| Solana | `solana:mainnet` | Ed25519 | First non-EVM profile, spec example and fixture published | [minimal-solana.json](https://github.com/neus/network/blob/main/examples/caip-380/minimal-solana.json) |
...
CAIP-380 is at Draft status in the official CASA registry. Advancing to Review and Accepted requires at least two independent implementations and editor advancement. See [HISTORY.md](https://github.com/neus/network/blob/main/HISTORY.md) for the full standards chronology.

...
NEUS's use and transfer of information received from Google APIs adheres to the [Google API Services User Data Policy](https://developers.google.com/terms/api-services-user-data-policy), including its Limited Use requirements.
...
You may add a remote MCP service or your own API key for an AI or tool provider. In that case, NEUS sends the information needed for your request directly to the service you selected. That provider's terms and privacy policy apply.
...
- NEUS-hosted inference uses RedPill, which routes supported models through confidential-computing infrastructure.

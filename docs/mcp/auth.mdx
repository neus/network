...
Some clients also follow a `401` + `WWW-Authenticate` challenge when a protected resource requires auth. Hosted NEUS MCP is open for public tools; sign in when you need account-aware tools such as **`neus_me`** or encrypted secrets.
...
| **Bearer (`npk_*` or OAuth JWT)** | Same header shape — `neus_me`, private reads, secrets tools |

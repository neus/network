---
title: MCP Auth & Security
description: Hosted auth-required behavior and eligibility-safe public MCP usage.
icon: lock
---

Public MCP tools expose only the data required for the task. This page defines what stays public and what requires authentication.

## Auth-Required Behavior

When a hosted MCP tool needs authentication:

- **`neus_me`** resolves the current principal through `GET /api/v1/auth/me` when auth is present.
- **`NEUS_AUTH_TOKEN`** is only the same NEUS session token in Bearer form for non-cookie contexts such as MCP or server runtimes.
- **Private proof access** stays behind NEUS auth boundaries; public MCP tools remain eligibility-safe.

<Warning>
Do not expect public MCP tools to expose private proof payloads or credentials.
</Warning>

## Eligibility-Safe Behavior

- Gate checks are **eligibility-safe**. They do not enumerate receipts.
- Private proof payloads are **not exposed** through the public MCP tools.
- Sponsor grants can be forwarded through `neus_proofs_check`.
- x402 payment-required responses are passed through in a structured way instead of being swallowed.

## Auth-Required Responses

When a tool needs authentication and none is present, the server can return:

- `status: "auth_required"` with a hosted login URL for the user to complete sign-in.
- Use the returned URL to hand off to `https://neus.network/verify?intent=login`.

## Related Docs

- [Authentication](../api/authentication) - API auth patterns and signing.
- [VerifyGate](../widgets/verifygate) - Widget gating and hosted checkout.
- [Verifiable Agents](../cookbook/verifiable-agents) - Agent identity and delegation.
- [Partner Setup](../cookbook/hub-integrator) - Domain verification and sponsor grants.

## Related

<CardGroup cols={2}>
  <Card title="MCP Overview" icon="plug" href="./overview">
    What the server is for.
  </Card>
  <Card title="Tools" icon="wrench" href="./tools">
    Tool reference and eligibility semantics.
  </Card>
  <Card title="Setup" icon="code" href="./setup">
    Hosted client config.
  </Card>
  <Card title="API Authentication" icon="key" href="../api/authentication">
    HTTP auth and signing.
  </Card>
</CardGroup>

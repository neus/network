...
Compatible interactive MCP clients authenticate through **OAuth 2.0 with PKCE**. The host can refresh the session for up to 30 days.
...
| Mode | Best for |
|------|----------|
| **OAuth** | Interactive clients: browser sign-in, no manual keys, silent refresh for 30 days |
| **Access key (`npk_*`)** | Servers, CI, and automation with stable environment variables. Durable and never expires |

Both modes send the same `Authorization: Bearer <token>` header against the same NEUS Profile and Account. `npk_*` keys are long-lived credentials. OAuth sessions are long-lived too. Your MCP host refreshes the short-lived access token silently for up to 30 days via the `offline_access` refresh token, exactly like Linear, GitHub, and Notion MCP hosts.

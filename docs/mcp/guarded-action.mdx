...
```bash
npx -y -p @neus/sdk neus mount guarded-demo --apply <host>
```

`<host>` is `cursor`, `claude`, or `codex`. The command writes the current `neus.runtime-mount.v1` bundle to `.neus/mount.json`.

...
**One NEUS MCP only.** Prefer the **neus-trust** plugin (OAuth). If the plugin is installed, keep `~/.cursor/mcp.json` without a `neus` entry (`{"mcpServers":{}}`). Do not run `neus setup --client cursor` on top of the plugin — that registers the same host twice.

Without the plugin, use setup **or** URL-only config (not both):

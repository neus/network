...
Installs the trust skill and configures hosted MCP for Cursor, Claude Code, Codex, and VS Code in one command. Marketplace plugins are an optional setup shortcut — never a second MCP registration.
...
Writes the single user-level registration at `~/.cursor/mcp.json`:
...
**Optional marketplace setup helper** (no MCP registration):

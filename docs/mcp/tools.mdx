...
| Topic | Detail |
|-------|--------|
| Bearer | Optional ([Auth](./auth)). With Bearer, call **`neus_me`**, then reuse existing proof receipts before creating anything. |
...
| Phase | Calls |
|-------|--------|
| Default | Connect → **`neus_context`** → **`neus_me`** if Bearer → **`neus_agent_link`** for agents → **`neus_proofs_check`** → continue if satisfied |

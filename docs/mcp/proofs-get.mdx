...
Use when you have an **`identifier`** and need proof records, fields, tags, or delegated context. In the reuse-first flow, call this after **`neus_proofs_check`** when you need details beyond yes/no eligibility. Profile totals and account context come from **`neus_context`** or **`neus_me`**.
...
Use `include: "content"` for a specific proof or tightly filtered page when the body is required. Use the paginated `proofs` list for history and the profile summary from `neus_context` or `neus_me` for aggregate totals. Never infer a total or absence from one page.

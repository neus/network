---
title: Setup
description: OAuth, access keys, and per-editor config after neus setup.
icon: code
---

Connect your editors to NEUS so they can load your account, reuse saved results, and use encrypted secrets.
...
Installs the trust skill, detects your editors, and signs you in. In Cursor, you can also install the `neus-mcp` plugin from the marketplace — it registers NEUS and runs sign-in in one click.
...
Per-editor setup commands (Cursor, VS Code, Claude Code, and more): **[NEUS Docs home](/)**.

The setup command connects **Cursor**, **Codex**, **VS Code**, and **Claude Code** and installs the trust workflow skill. Other MCP hosts use the same hosted URL — see [IDE plugin](./ide-plugin).

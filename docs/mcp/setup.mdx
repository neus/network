...
Connect any MCP client to NEUS: agents, editors, servers, CI, APIs. One endpoint loads your identity, permissions, proofs, and Vault.
...
Installs the NEUS skill and configures hosted MCP for your clients. It does not launch a browser. Sign in by clicking **Connect** in your host or run `neus auth` separately.

## Daily commands

| Step | Command |
|------|---------|
| Connect this machine | `npx -y -p @neus/sdk neus setup` |
| Connect an agent to this project | `neus mount <agentId> --apply cursor` |
| Health check | `npx -y -p @neus/sdk neus doctor --live` |
| Read proofs | MCP tool `neus_proofs_get` |

## Access keys

OAuth is the default for client setup. Your MCP host discovers the NEUS OAuth metadata on first use and refreshes your session silently for up to 30 days.
...
| Goal | Command | Output |
|------|---------|--------|
| **Connect a trusted agent to this project** | `neus mount <agentId> --apply cursor` | `.neus/mount.json` + host rules (live proofs) |
...
Per-client setup commands and config paths: [MCP clients](./ide-plugin).

The setup command installs the NEUS skill and configures hosted MCP. Other MCP clients use the same hosted URL. See [MCP clients](./ide-plugin).

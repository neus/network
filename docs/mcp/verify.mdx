---
title: "Create proof (`neus_verify`)"
sidebarTitle: "Create proof"
description: "Create a saved proof from inside an MCP session."
icon: circle-check
---

Use **`neus_verify`** to create a proof when the signed-in account can complete the check in-session. For steps that need a browser (OAuth, personhood, payment, wallet connect), use **[`neus_verify_or_guide`](./verify-or-guide)** instead. It returns a hosted link.
...
| `options.publishToHub` | No | Make the proof discoverable on your profile |
...
A plain-language **Passed**, **Action needed**, or **Blocked** result. On success, the tool also returns the proof ID in `qHash` for your server. If a browser step is required, it returns a `next_action` link.

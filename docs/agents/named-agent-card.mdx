...
Every registered NEUS agent has a **public card** — a discoverable profile that shows its identity, permission scope, and capabilities. Other apps and agents can check an agent before trusting it.
...
Sensitive fields (`allowedActions`, `deniedActions`, `approvalPolicy`) are owner-view only — they are not exposed in public card responses. External consumers see the agent's identity, capabilities, and controller, but not the full permission lists.

## How to use it

1. **Discover** — fetch the agent card to learn what the agent is and who controls it.
2. **Verify** — check the permission receipt to confirm the controller trusts the agent wallet.
3. **Delegate** — if building on NEUS, create your own permission grant to the agent for your app's scope.

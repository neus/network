...
`<host>` is `cursor`, `claude`, or `codex`. VS Code uses `--apply cursor`. Run the command in your project. It writes `.neus/mount.json` and the host adapter. Start a new agent session so the project rules load. If the host is not connected yet, run `neus setup` first. The browser never downloads a static rules file.

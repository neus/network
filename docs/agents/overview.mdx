...
Same onboarding as any integration: **[Get started](../get-started)** (`appId`, credits, billing), then agent verifiers below.
...
<Frame caption="Register the agent, delegate from the controller wallet, then operate with proof receipts.">
  <img
    className="neus-docs-flow-diagram"
    src="/images/diagrams/agents-trust-loop.svg"
    alt="Agent flow: agent-identity registration, agent-delegation from controller, then actions with proof receipts"
  />
</Frame>

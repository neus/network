...
<Steps>
  ...
  <Step title="Create">
    Call **`neus_agent_create`** with **`agentId`** (required; NEUS auto-generates one if omitted) and **`agentWallet`**. Finish on NEUS via **`hostedVerifyUrl`** or the returned signing steps.
  </Step>
  <Step title="Confirm">

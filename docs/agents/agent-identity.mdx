...
Register an agent; agent wallet signs. Proof `skills` are **metadata only**—they **do not** install MCP servers — [MCP overview](../mcp/overview). Prefer portable skill ids ([spec](https://agentskills.io/specification), [directory](https://skills.sh)).
...
Proof receipt id. **Signed with the agent wallet.** **No spend or permission grants** — use [`agent-delegation`](./agent-delegation) for scoped authority.

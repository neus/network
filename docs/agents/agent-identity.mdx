...
**MCP vs proof skill ids:** declarative `skills` on the proof are not MCP installs — see [MCP overview](../mcp/overview). Prefer **Open Agent Skills** ids ([spec](https://agentskills.io/specification), [directory](https://skills.sh)) so hosts can resolve the same `SKILL.md` packs.
...
await client.verify({
  ...
  data: {
    ...
    skills: ['web-search', 'vercel-react-best-practices'],   // max 48, each max 64 chars; hyphen ids match agentskills.io
    services: [                                  // max 16

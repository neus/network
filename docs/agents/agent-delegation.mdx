...
One-time user approval so your backend can create proofs without asking them to sign every request. This is **not** the same as **Register app** in Hub (billing for hosted checks). See [Server integrations (advanced)](../cookbook/server-advanced).

...
Delegation receipts are the authorization layer for agents: who approved this agent, what it may do, and how much it can spend — all checkable by any gate.
...
One-time user approval so your backend can create proofs without asking them to sign every request. This is **not** the same as creating a **portal** in your profile (billing for hosted checks). See [Integrations](../cookbook/integrations).

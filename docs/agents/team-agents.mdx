...
description: Approve many agents from one account while giving each agent different authority.
icon: user-group
---
...
<Steps>
  ...
  <Step title="Run under the right agent">
    Point each project or product workflow at the matching `agentId` so actions run under that agent's authority. Scheduled jobs live in the product UI when enabled for your account. They are not part of the twelve public MCP tools.
  </Step>
</Steps>

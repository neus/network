...
description: NEUS Portable Trust Infrastructure for humans and AI.
icon: book-open
...
<Frame caption="**Figure 5.** `qHash` can correlate a CAIP-380 signed request with a NEUS proof, but request integrity and verifier outcome remain separate claims.">
  <img src="/images/whitepaper/figure-5-envelope-and-receipt.svg" alt="CAIP-380 request envelope and NEUS proof: the signed envelope proves request integrity, the NEUS proof records the verifier outcome and lifecycle state, qHash correlates them but does not prove the result by itself." />
</Frame>

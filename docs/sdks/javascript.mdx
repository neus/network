...
Use the SDK when your app needs direct control over proof creation, hosted handoff, and status polling without raw HTTP request assembly.
...
// proofId is the primary SDK field for this receipt.
// In HTTP endpoints, the same identifier may appear as {qHash}.
...
| Option | When to Use |
|--------|-------------|
| `persist: true` | Store the proof when you need a reusable proof receipt |
...
const res = await client.gateCheck({
  ...
  contractAddress: '0x...',
  minBalance: '100',
  chainId: 8453, // Required for asset verifiers (NFT, token, contract) — specifies the chain the asset lives on

...
Use `@neus/sdk` instead of ad-hoc `fetch` to `api.neus.network` so headers and paths stay correct.
...
| Server | **`gateCheck`** with `appId` + `billingWallet`; advanced: **`verifyFromApp`**, access keys |
...
`getHostedCheckoutUrl` supports `gateId`, `returnUrl`, `verifiers`, `preset`, `mode`, `intent`, `origin`, `oauthProvider`. The hosted page also accepts `returnTo` and `connect=1` (append manually). [Hosted Verify](../cookbook/auth-hosted-verify).
...
Set `appId` on the client for usage attribution. It is public, not a secret.
...
[Verification patterns](./verifications) for privacy and widget options.

...
`getHostedCheckoutUrl` supports `gateId`, `returnUrl`, `mode`, `intent`, `origin`, `oauthProvider`. The hosted page also accepts `returnTo` and `connect=1` (append manually). [Hosted Verify](../cookbook/auth-hosted-verify).

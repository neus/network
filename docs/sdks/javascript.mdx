...
`getHostedCheckoutUrl` supports `gateId`, `verifiers`, `preset`, `returnUrl`, `mode`, `intent`, `origin`, `oauthProvider`, `appId`, `billingWallet`, and `baseUrl`. The hosted page also accepts `returnTo` and `connect=1` (append manually). [Hosted Verify](../cookbook/auth-hosted-verify).
...
| `publishToHub: true` | Make the receipt discoverable in Hub |

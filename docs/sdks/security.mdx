...
Less surface area: interactive → hosted; private → owner auth.
...
`client.verify()` → **private**, `storeOriginalContent: true`. VerifyGate create → **unlisted public**. [Security and trust](../platform/security-and-trust)

SDK + `gateCheck` without owner session → set unlisted public explicitly:

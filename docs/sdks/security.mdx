...
The SDK defaults to **`private`** receipts with **`storeOriginalContent: true`** (private stored). For **reusable gates** and **`gateCheck`** without an owner session, set **unlisted public** explicitly:
...
<Note>
Default is **private stored**. Use the block above when you need the same receipt to power gates and server checks without the owner signed in. Unlisted public proofs are still public to anyone with the proof id.
</Note>

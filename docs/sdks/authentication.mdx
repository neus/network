...
For Cursor, VS Code, or Claude Code, run `neus setup` then `neus auth`. That stores a separate MCP access credential for `https://mcp.neus.network/mcp` — not the browser session cookie from `neus.network/verify`. Use a profile access key (`npk_*`) for servers and CI when browser sign-in is unavailable.

...
description: Reuse, fresh, hosted checkout, and server checks.
icon: network-wired
---
...
`client.verify()` is an advanced server or controlled-signing path. `VerifyGate` create opens **Hosted Verify** and uses the published gate policy. Full matrix: [Security and trust](../platform/security-and-trust)
...
```jsx
<VerifyGate strategy="fresh" gateId="gate_abc123">
  <Protected />
</VerifyGate>
```
...
```jsx
<VerifyGate strategy="reuse" gateId="gate_abc123">
  <Protected />
</VerifyGate>
```

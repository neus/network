...
A wallet-signed creation response also includes `portableProof`, a [CAIP-380](../learn/standards/caip-380) portable-proof envelope that another system can verify without calling NEUS. Save that envelope if portability matters; NEUS does not persist the complete request payload by default. Session-authorized receipts have `receipt.portable: false` and remain reusable through NEUS receipt APIs.
...
See [Integration](../integration) for the full surface.
...
| `portableProof` when returned | Complete private payloads in public storage |

...
When a check finishes, NEUS returns a **saved result**. Store the proof ID. That is how you look it up, share it, and reuse it.

In API responses the field is `qHash`. On product pages, call it the proof ID.
...
| | Role |
|--|--|
| Proof ID (`qHash`) | Reference in SDK responses, API paths, and URLs |

`contentHash` identifies the content or artifact. It is not the proof ID.

Visibility: [Privacy](./security-and-trust)

## What to store

Save the proof ID and any fields your product needs for access, display, or audit. Do not rebuild the full check on every request. Look up the current result instead.

...
description: Pay per call with x402 v2 headers and USDC on Base — no account required.
icon: coins
---

x402 lets an application or agent pay for a NEUS operation without creating an account or API key. Three resources support pay-per-call:
...
The exact price for each request is in the `PAYMENT-REQUIRED` header — it scales with the number of verifiers, query complexity, and optional add-ons (IPFS pinning, extra chains).

## Flow

<Steps>
  <Step title="Request the resource">
    Send the normal request with the required parameters (address, verifier IDs, or gate ID).
  </Step>

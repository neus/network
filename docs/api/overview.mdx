...
Use the HTTP API from **servers, workers, CLIs, and MCP**—not from end-user browsers. Responses use the same **`qHash`** as the SDK; usage is attributed to the NEUS profile linked to your app.

**Base URL:** `https://api.neus.network`

## Browsers

Send end users through the **[JavaScript SDK](../sdks/javascript)** or **[Hosted Verify](../cookbook/auth-hosted-verify)**. If a web client needs behavior outside those paths, **your backend** should call NEUS so secrets stay off the client and CORS stays predictable.
...
| `/api/v1/verification` | POST | Create proof (advanced) |

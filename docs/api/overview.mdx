...
> **Scope note:** This reference covers the core HTTP proof and verification surface. Gate endpoints (`/api/v1/profile/gates/{gateId}`, `.../fulfill`, `/api/v1/profile/gates/discoverable`) are product checkout endpoints documented in [Hosted Gate Checkout](../platform/hosted-gate-checkout). Standards-native x402 v2 payment for `GET /api/v1/proofs/check` is documented in [x402](../platform/x402).
